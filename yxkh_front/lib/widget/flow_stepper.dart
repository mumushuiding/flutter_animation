...
class FlowStepperWidget extends StatelessWidget {
  ...
  FlowStepperWidget({Key key, this.data})
      : assert(data != null),
        super(key: key);
  ...
  List<Step> getSteppers() {
        ...
        title: Wrap(
          children: <Widget>[
            Padding(
              ...
              child: CircleAvatar(
                radius: 20,
                backgroundImage: NetworkImage(data["ApplyUserImage"]),
              ),
            ),
            Padding(
              padding: EdgeInsets.all(2),
    ...
    data["ApprovalNodes"].forEach((d) {
      steppers.add(Step(
          title: Column(children: getNames(d["Items"]["Item"])),
      ...
      return Row(
        children: <Widget>[
          Padding(
            ...
            child: CircleAvatar(
              radius: 15,
              backgroundImage: NetworkImage(a["ItemImage"]),
            ),
          ),
          Padding(
            padding: EdgeInsets.all(2),
          ...
          Padding(
            padding: EdgeInsets.all(2),
            child: Text("${getStatus(a["ItemStatus"])}", style: TextStyle(fontSize: 12)),
          ),
        ],
      );
    }).toList();

...
class ResponsiveTable extends StatelessWidget {
  ...
  ResponsiveTable({Key key, this.header, this.actions, this.datas, this.columns, this.rowsPerPage = 10, this.operation})
      : assert(columns != null),
        super(key: key);
  ...
  List<DataCell> getDataCells(int index) {
    ...
    columns.forEach((c) {
      switch (c.key) {
        ...
        default:
          list.add(
              DataCell(Text("${c.formatter == null ? (data[index][c.key] ?? '') : c.formatter(data[index][c.key])}")));
      }
    });
    return list;
  }
